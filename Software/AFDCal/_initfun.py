...
def setDecompMethod(self,
    ...
    if type(decompMethod) is int:
        ...
        if decompMethod.lower() == 'Single Channel Conventional AFD'.lower():
            ...
            self.decompMethod = 2
        elif decompMethod.lower() == 'Multi-channel Conventional AFD'.lower():
            self.decompMethod = 3
        elif decompMethod.lower() == 'Multi-channel Fast AFD'.lower():
            self.decompMethod = 4
...
def loadInputSignal(self, 
    ...
    if np.isreal(self.s).all():
        self.G = np.zeros(self.s.shape, 'complex')
        for i_ch in range(self.s.shape[0]):
            self.G[i_ch,:] = pysig.hilbert(self.s[[i_ch],:])
    else:
        self.G = self.s.copy()
    # set phase
...
def setPhase(self, 
    ...
    if t.ndim != 2:
        raise ValueError("The dimension of the given signal phase must be C*N where N is the sampling number of the input signal, and C is the channel number of the input signal")
    if t.shape[0] != self.s.shape[0] or t.shape[1] != self.s.shape[1]:
        raise ValueError("The dimension of the given signal phase must be C*N where N is the sampling number of the input signal, and C is the channel number of the input signal")
