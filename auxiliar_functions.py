...
def predictFMM(a, phis, time_points):
    ...
    prediction = np.ones((n_ch, n_obs), dtype = complex)*phis[:,0:1]
...
def transition_matrix(an):
    ...
    """
    K = len(an)  # Length of the an vector
    M = np.zeros((K+1, K+1), dtype=complex)  # Initialize the (K+1)x(K+1) matrix with zeros
    xiMat = calculate_xi_matrix(an)  # Call the xiMatrix function
    ...
    for k in range(K):
        ...
        M[0, k+1] = (an[k] * xiMat[k, 0] + xiMat[k+1, 0]) / np.sqrt(1 - np.abs(an[k])**2)

        # Triangular matrix: only upper triangle is not zero. Each k is a row.
        for j in range(k+1, K):
            M[k+1, j+1] = (np.sqrt(1 - np.abs(an[j])**2) * xiMat[j, k+1] /
                           (np.conj(an[k]) - np.conj(an[j])))

        # Diagonal elements of M
        M[k+1, k+1] = xiMat[k+1, k+1] / np.sqrt(1 - np.abs(an[k])**2)
